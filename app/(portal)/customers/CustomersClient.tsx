...
import type { CustomerListRow } from "@/lib/customers-list";
...
export const CUSTOMERS_SEED_URL = "/api/customers?q=&page=1&pageSize=10";

type CustomersClientProps = {
  user: SessionUser;
  initialList: Customer[];
...
export default function CustomersClient({
  ...
  const [list, setList] = useState<Customer[]>(initialList);
  const [q, setQ] = useState("");
  const [ownerQ, setOwnerQ] = useState("");
  const [status, setStatus] = useState("");
  const [createdFrom, setCreatedFrom] = useState("");
  const [createdTo, setCreatedTo] = useState("");
  const [canOwnerSearch] = useState(() => canShowOwnerSearch(user));
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [meta, setMeta] = useState<PageMeta>(initialMeta || EMPTY_PAGE_META);
  ...
  /** 默认首屏已由服务端灌入，跳过首轮相同条件的客户端请求 */
  const skipDefaultFetch = useRef(true);

  useEffect(() => {
    pageCachePut(CUSTOMERS_SEED_URL, {
      data: initialList,
      meta: initialMeta,
    });
  ...
  const load = useCallback(
    async (opts?: {
      ...
      const cached = pageCachePeek<{ data?: Customer[]; meta?: PageMeta }>(url);
  ...
  useEffect(() => {
    ...
    const isDefault =
      ...
      !createdFrom &&
      !createdTo;
    if (skipDefaultFetch.current && isDefault) {
