...
含义：`/_next/static/...` 实际是 **404/错误页（text/plain）**，不是真正的 CSS/JS。

### 静态资源与平台代理（重要）

```
浏览器 → Coze 平台代理 → 容器内 Next（0.0.0.0:5000）
```

平台会把公网请求转发到容器。若容器内 **没有** 正确提供 `/_next/static`（文件缺失、standalone 未拷贝、服务未监听等），代理常返回 **纯文本 404**，`Content-Type` 变成 `text/plain`，浏览器就会报上述 MIME 错误——看起来像「代理改坏了类型」，实质是 **上游没给出正确静态文件**。

应对原则：

1. 容器内用 **`next start` / `next dev`** 直接托管 `/_next/static`（不要裸跑缺 static 的 standalone）
2. 监听 **`0.0.0.0:$PORT`**（预览/部署多为 **5000**），让平台代理能打到进程
3. 验收时直接打开 `https://域名/_next/static/css/...`，确认 **200** 且类型含 `css`/`javascript`

根因优先查：**静态目录是否随运行环境提供**，而不是去改浏览器 MIME 设置。
...
| 现象 | 处理 |
|------|------|
| CSS/JS MIME `text/plain` / 404 | 去掉 standalone；用 `next start`/`next dev`；确认代理打到 5000；完整重新部署 |
| 平台代理后静态仍 text/plain | 先 curl 容器内 `localhost:5000/_next/static/...`；容器内 404 则不是代理单独改 MIME |
...
请按本文修改项目：删除 `output: "standalone"`；使用 `next dev`/`next start` 监听 `0.0.0.0` 与平台端口；CSP 允许 `https://lf-cdn.coze.cn`、`https://apm.volccdn.com`、`https://fonts.bytedance.com`，且 `'unsafe-inline'` 与 `'unsafe-eval'` 之间保留空格；生产配置 `DATABASE_URL`（含 sslmode）、`SESSION_SECRET`、`CONFIG_ENCRYPTION_KEY`、`APP_ORIGINS`、`TRUST_PROXY=true`；确保 `/_next/static` 在开发与生产均可 200 加载。
