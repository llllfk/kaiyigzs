...
const navy = "#17324D", teal = "#167D83", pale = "#EAF3F3", border = "#D8E0E6";
...
overview.getRange("A3:H3").values = [["统计范围",data.scopeLabel,"统计开始",data.from,"统计结束",periodEndText,"生成时间",new Date().toLocaleString("zh-CN")]];
...
const sales=wb.worksheets.add("销售人员"); title(sales,`${data.periodLabel}销售人员表现`,"E"); table(sales,3,["销售人员","新增客户","跟进次数","新增商机","商机金额"],data.sales.map(x=>[x.name,num(x.new_customers),num(x.follow_ups),num(x.opportunities),num(x.opportunity_amount)]),{E:"¥#,##0.00"}); sales.getRange("E:E").format.columnWidth=20;
